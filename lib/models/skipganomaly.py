...
class Skipganomaly(BaseModel):
    ...
    def __init__(self, opt, data=None):
        ...
        self.noise = torch.empty(size=(self.opt.batchsize, 1, self.opt.isize, self.opt.isize), dtype=torch.float32, device=self.device)
    ...
    def test(self, plot_hist=True):
        ...
        with torch.no_grad():
            ...
            self.an_scores = (self.an_scores - torch.min(self.an_scores)) / \
                             (torch.max(self.an_scores) - torch.min(self.an_scores)).cpu()
            auc = roc(self.gt_labels, self.an_scores)
            performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('AUC', auc)]) # 实现字典元素的排序
            ...
            if plot_hist:
                ...
                hist.to_csv("hero_predict_result.csv")
    ...
    ##
    def test_one_epoch(self, plot_hist=True):
        """ Test GANomaly model.
